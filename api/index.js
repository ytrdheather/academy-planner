...
app.post('/api/generate-daily-comment', requireAuth, async (req, res) => {
    ...
    try {
        ...
        [말투 — 가장 중요]
        - 너는 학습 전문가(담임 교사)다. 정중하고 담담한 문어체로 쓴다. 문장 대부분을 '~습니다 / ~합니다 / ~ㅂ니다 / ~보입니다'로 끝맺는다. '~요'로 끝나는 문장(예: ~했어요, ~예요)은 글 전체에서 최대 1~2문장까지만 허용한다. 나머지는 전부 '~니다' 체로 쓴다.
        ...
        [사실 왜곡 절대 금지 — 매우 중요]
        - 키워드에 적힌 사실을 각색하거나 부풀리지 마라. 특히 "~까지 ~해오기/풀어오기"는 '앞으로 해야 할 숙제 부여'다. 이것을 "잘 해왔다"처럼 완료된 일로 절대 바꾸지 마라.
        - 키워드에 없는 에피소드·감정·상황을 지어내지 마라. 키워드가 짧으면 코멘트도 짧게 쓰는 것이 옳다.
        ...
        1문단: "오늘의 리디튜더 ${studentName}의 일일 학습 리포트📑를 보내드립니다." 로 시작 → 한 줄 띄우고 → 키워드 내용을 사실 그대로, 차분하게 서술. (키워드가 "없음"이면 이 본문은 생략하고 바로 2문단으로)
        ...
        오늘 지민이는 독해 지문에서 헷갈렸던 구문을 다시 확인하고, 오답을 스스로 정리했습니다. 모르는 부분을 그냥 넘기지 않는 태도가 인상적이었습니다.

        <📢 오늘의 학습 결과와 숙제 수행율 안내>
        오늘 어휘 테스트는 95점으로 안정적인 결과를 보였고, 독해 해석 시험도 무난히 통과했습니다. 숙제 수행율은 100%로, 맡은 분량을 꾸준히 해내고 있어 믿음직스럽습니다.
        ...
        // 차분한 전문가 톤 유지: temperature를 낮춰 과장·호들갑 억제. 추론 1024 유지.
        const result = await geminiModel.generateContent({
            contents: [{ role: 'user', parts: [{ text: prompt }] }],
            generationConfig: { temperature: 0.6, maxOutputTokens: 2500, thinkingConfig: { thinkingBudget: 1024 } }
        });
