        ...
        for (const student of students) {
            ...
            try {
                ...
                if (geminiModel) { // [수정] comments가 비어있어도 AI가 통계 기반으로 쓰도록
                    try {
                        // ▼ [수정] AI 프롬프트 교체 (ai_prompt_directions.md의 9가지 지침)
                        // (헤더님이 보내주신 최신 프롬프트로 교체)
                        const prompt = `
...
2.  마치 헤더쌤이 학부모님께 카톡을 보내는 것처럼, "어머님(아버님), 안녕하세요. ${studentName} 학생 10월 리포트 보내드립니다."처럼 자연스럽고 친근하게 첫인사를 시작해 줘.
...
3.  **[정성 평가]** "Dora's Mystery..."를 읽은 것처럼 긍정적인 부분이 있다면, **그것을 먼저 칭찬**하면서 코멘트를 시작해 줘. (예: "이번 달에 ${studentName} 학생이 'Dora's Mystery' 원서를 1권 완독했네요! 정말 기특합니다.")
