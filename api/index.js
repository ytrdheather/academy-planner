...
const NOTICE_FORMS = [
    ...
    { label: '결석 · 지각 · 조퇴', desc: '미리 알려주시면 보강 일정을 잡아드립니다', url: process.env.FORM_ABSENCE_URL || '/absence' },
...
app.post('/api/absence', async (req, res) => {
    ...
    // 지각이면 예상 도착 시간, 조퇴면 나가는 시간. 자유 텍스트라 적으신 그대로 전달된다.
...
try {
    initializeMonthlyReportRoutes({
        app, cron, fetchNotion, geminiModel, requireAuth,
...
// 노션 스키마 드리프트 점검 — 속성 이름이 사라지면 그 기능이 통째로 죽는데 화면엔 안 나온다.
// 매일 07:30 에 대조하고 어긋난 것만 원장 DM 으로 올린다. → api/notionSchema.js
try {
    initializeSchemaCheck({
        app, cron, requireAuth, fetchNotion,
        dbIds: {
            STUDENT_DATABASE_ID, PROGRESS_DATABASE_ID, GRAMMAR_DB_ID, MONTHLY_REPORT_DB_ID,
            ENG_BOOKS_ID, KOR_BOOKS_ID, TEXTBOOK_UNIT_DB_ID, EXAM_DB_ID, STUDENT_RESULT_DB_ID,
            ABSENCE_DB_ID, COUNSEL_DB_ID, NOTICE_DB_ID, PAUSE_DB_ID,
            COUNSEL_LOG_DB_ID: process.env.COUNSEL_LOG_DB_ID || '3c509320-bce2-819e-9709-fb72ace74bf8',
            ADMISSION_DB_ID: process.env.ADMISSION_DB_ID || '1a109320-bce2-8042-b1b8-d13661def917',
        },
        notifyOwner: KAKAOWORK_APPROVAL_CONV
            ? (title, body) => sendKakaoWork(KAKAOWORK_APPROVAL_CONV, `[${title}]\n\n${body}`)
                .catch(e => console.error('스키마 점검 통지 실패:', e.message))
            : null,
    });
} catch(e) { console.error('Schema Check Init Error', e); }
...
// 데일리 리포트 — 진도 행 생성 · 리포트 HTML · 학생 본인 조회 · URL 채우기.
// 크론 10:20(생성) · 22:00(URL). index.js 를 더 키우지 않으려고 통째로 뺐다.
try {
