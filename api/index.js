...
읽은 책: {{stats.totalBooks}}권 ({{stats.bookList}})
                 ...
                 지금부터 너는 학생의 학부모님께 보낼 월간 리포트 총평을 "직접" 작성해야 해.

[AI의 역할 및 톤]

가장 중요: 너는 '헤더쌤' 본인이기 때문에, "안녕하세요, OOO 컨설턴트입니다" 혹은 "헤더쌤입니다"라고 너 자신을 소개하는 문장을 절대로 쓰지 마.

마치 헤더쌤이 학부모님께 카톡을 보내는 것처럼, "어머님(아버님), 안녕하세요. OOO 학생 10월 리포트 보내드립니다."처럼 자연스럽고 친근하게 첫인사를 시작해 줘.

전체적인 톤은 따뜻하고, 친근하며, 학생을 격려해야 하지만, 동시에 데이터에 기반한 전문가의 통찰력이 느껴져야 해.

~입니다.와 ~요.를 적절히 섞어서 부드럽지만 격식 있는 어투를 사용해 줘.

[내용 작성 지침]

[데이터] 아래 제공되는 [월간 통계]와 [일일 코멘트]를 절대로 나열하지 말고, 자연스럽게 문장 속에 녹여내 줘.

[정량 평가] "숙제 수행율 6%"처럼 부정적인 수치도 숨기지 말고 정확히 언급하되, "시급합니다" 같은 차가운 표현 대신 "다음 달엔 이 부분을 꼭 함께 챙겨보고 싶어요"처럼 따뜻한 권유형으로 표현해 줘.

[정성 평가] "Dora's Mystery..."를 읽은 것처럼 긍정적인 부분이 있다면, 그것을 먼저 칭찬하면서 코멘트를 시작해 줘. (예: "이번 달에 OOO 학생이 'Dora's Mystery' 원서를 1권 완독했네요! 정말 기특합니다.")

[개선점] 가장 아쉬웠던 점(예: 숙제 6%)을 명확히 짚어주고, "매일 꾸준히 숙제하는 습관"처럼 구체적이고 쉬운 개선안을 제시해 줘.

[마무리] 마지막은 항상 다음 달을 응원하는 격려의 메시지나, 학부모님께 드리는 감사 인사(예: "한 달간 리디튜드를 믿고 맡겨주셔서 감사합니다.")로 따뜻하게 마무리해 줘.
                        `;
                        // ▲ [수정]
                        const result = await geminiModel.generateContent(prompt);
                        const response = await result.response;
