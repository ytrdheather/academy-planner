 ...
 *    독촉은 **화요일 20:00 주 1회**만 돈다(2026-09-15 원장 확정). 학부모 발송은 금요일 21시이므로
 *    첫 화요일엔 3.96일, **두 번째 화요일엔 10.96일**이 지난다. 10이면 정확히 두 번째 화요일에 잡힌다.
 *    (월요일 11:10 이던 때는 두 번째 월요일이 9.6일이라 9로 낮춰야 했다 — 10을 넣으면 그 회차가
 *    조용히 건너뛰어지고 일주일이 통째로 밀렸다. 시각을 옮길 때마다 이 계산을 다시 할 것.)
 *    뜻은 "발송 후 두 번째 화요일".
 */
const UNPAID_AFTER_DAYS = 10;
