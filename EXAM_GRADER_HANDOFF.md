...
### ✅ 완료: 재채점 (커밋 `리포트 캐싱` 이후)
- `POST /api/regrade-exam` — 해당 시험 응시 **학생 전원 재채점**: 저장된 `학생답`을 **현재 정답지**와 다시 대조해 **객관식만** 재계산, **선생님이 매긴 서술형 점수(부분점수 포함) 보존**. STUDENT_ANSWER 행 + STUDENT_RESULT 총점 갱신. 재채점 시 캐시된 `AI코멘트`를 비워 리포트가 새 데이터로 재생성되게 함.
- 점수 집계를 순수 함수 `computeResultSummary` + 속성 빌더 `buildResultProps`/`buildAnswerProps`로 공통화(저장·재채점 공유).
- `results-viewer.html` 시험별 조회에 **🔄 이 시험 전체 재채점** 버튼 추가.
- 검증: 정답지 수정 시 오답→정답 전환, 서술형 점수 보존, 총점 재계산 유닛테스트 통과.
- **사용법**: Notion에서 정답지(QUESTION_DB) 정답 수정 → 결과 조회 → 해당 시험 선택 → 🔄 재채점 클릭.

### 🔜 남은 것
1. **정답지 수정 화면**(선택) — 지금은 Notion에서 직접 수정. 앱 안에서 고치는 화면이 있으면 편함: `GET /api/exam-answer-key` + `POST /api/update-answer-key`(QUESTION_DB 업데이트) + UI.
2. **서술형 채점/수정 화면** — 학생 셀프 제출분의 서술형을 원장이 채점해야 함. `results-viewer.html`에서 저장된 결과를 열어 수정하는 화면 필요(`GET result-answers` + `POST update-student-result`). 완료된 부분점수 UI(`student-grader.html` 방식) 재활용.

각 단계 커밋 후, 배포하려면 `main`에 병합.
