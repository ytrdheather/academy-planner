---
id: cron-jobs
title: 크론 잡 전체 (17개)
type: entity
status: verified
source: api/index.js, api/schemaCheckModule.js, api/dailyReportModule.js, api/confirmNotifyModule.js, api/textbookFeeModule.js, api/monthlyReportModule.js, api/admissionModule.js, api/arrivalAlertModule.js
updated: 2026-09-04
tags: [cron, schedule, automation]
---

## 정체

`node-cron`으로 서버 프로세스 안에서 도는 스케줄 17개. **전부 `{ timezone: 'Asia/Seoul' }`을 명시**하므로 아래 시각은 한국 시간이다.

## 표

| 시각(KST) | cron | 위치 | 하는 일 |
|---|---|---|---|
| 5분마다 | `*/5 * * * *` | `textbookFeeModule.js:997` | 교재비 tick — 원장 알림·발송·보류·정리 |
| 5분마다 | `*/5 * * * *` | `admissionModule.js:162` | 신입생 상담 예약확인 알림톡. 노션 `발송` 체크박스가 방아쇠 |
| 5분마다 | `*/5 * * * *` | `confirmNotifyModule.js:369` | 보강 확정 + 통화 확정 알림톡. 노션 `확정발송` 체크가 방아쇠 |
| 04:00 매일 | `0 4 * * *` | `confirmNotifyModule.js:386` | 지난 보강·지각·상담 건 자동 마감 |
| 07:30 매일 | `30 7 * * *` | `schemaCheckModule.js:128` | 노션 속성이 사라졌나 대조. 어긋난 것만 원장 DM → [[schema-check]] |
| 08:00 매일 | `0 8 * * *` | `confirmNotifyModule.js:378` | 그날 보강 명단 발송 (없으면 조용) |
| 09:00 매월 1일 | `0 9 1 * *` | `monthlyReportModule.js:757` | **지난달** 월간 리포트 전원 생성. 🔴 월 1회뿐 — 놓치면 다음 기회가 한 달 뒤 |
| 10:00 월요일 | `0 10 * * 1` | `textbookFeeModule.js:1116` | 조교 장보기 목록 → `KAKAOWORK_ASSISTANT_CONV` |
| 10:20 매일 | `20 10 * * *` | `dailyReportModule.js:358` | 데일리 리포트 행 자동 생성. 정지 기간이면 건너뜀 → [[daily-report]] |
| 11:00 매일 | `0 11 * * *` | `api/index.js:3322` | 숙제 자동 생성. 정지 기간이면 건너뜀 |
| 11:10 토요일 | `10 11 * * 6` | `textbookFeeModule.js:1075` | 교재비 **토요일 재발송** — 금요일 밤 늦게 승인돼 21시를 놓친 `승인됨` 건. 0건이면 조용. 나간 게 있으면 담당쌤 알림도 이어서 |
| 14:00 평일 | `0 14 * * 1-5` | `textbookFeeModule.js:1094` | 교재비 **반려** 알림만. 승인 건은 여기서 안 나간다 → 금 21시 |
| 16:00 매일 | `0 16 * * *` | `api/index.js:717` | 방치된 상담 건 리마인드 (담임 → 없으면 원장) |
| 20:00 화요일 | `0 20 * * 2` | `textbookFeeModule.js:1106` | 교재비 ① **`승인됨` 묶음 발송**(0건이면 조용, 나간 게 있으면 담당쌤 알림) → ② **1차 미입금 독촉** (발송 후 두 번째 화요일, 10.96일. 학부모 1회 + 미수금 채널·이명수 DM) |
| 21:00 금요일 | `0 21 * * 5` | `textbookFeeModule.js:1057` | **교재비 학부모 묶음 발송** → 끝나면 같은 콜백에서 **담당쌤 주간 알림**을 이어서 보낸다 |
| (일회성) | `TEXTBOOK_ONESHOT_AT` | `textbookFeeModule.js:1026` | 금 21시를 놓쳤을 때 **한 번만** 묶음 발송. 5분 크론에 얹혀 있다 |
| 22:00 매일 | `0 22 * * *` | `dailyReportModule.js:281` | 그날 진도 행에 `데일리리포트URL` 채우기 |
| 매시 15·45분 | `15,45 * * * *` | `arrivalAlertModule.js:253` | 미도착 알림 — 등원 시각+15분 지났는데 출석 미체크. 09~23시만, 0명이면 조용 |

## 쓰는 곳

주간 리듬이 요일로 갈려 있다 — 선생 신청(월~목 밤10시) → 원장 승인(금) → 학부모 발송(금 21시) → 조교 장보기(월·화). 시각 하나를 바꾸면 이 사슬이 흔들린다. [[textbook-fee]] 참고.

🔴 **매월 1일 09:00 만 유일하게 월 단위다.** 나머지는 놓쳐도 하루~일주일 뒤에 다시 오지만 이건 한 달 뒤다. 배포와 겹치면 반드시 `POST /api/monthly-report/tick` 으로 복구한다 → [[monthly-report]]

교재비 담당쌤 알림은 **크론 두 개로 갈려 있다**(2026-09-04 원장 확정). 반려는 평일 14시에 빨리, 승인 결과는 금요일 학부모 발송 직후에 몰아서. 금요일 것은 크론을 새로 만들지 않고 21시 발송 콜백 끝에 이어 붙였다 — "발송 뒤"를 순서로 보장하려는 것이다. → [[textbook-fee]]

5분 크론 3개는 **노션 체크박스가 방아쇠**인 구조다. 사람이 노션에서 체크 → 5분 안에 나감. 즉시 발송이 필요하면 대응하는 `/api/*/tick` 라우트를 직접 부르면 된다.

## 일회성 발송 (`TEXTBOOK_ONESHOT_AT`)

금요일 21시 배치를 놓치는 일이 **세 번** 있었다 — 2026-08-07(원장이 22:45 에 승인), 2026-08-21(21시에 전부 `승인대기`), 2026-09-11(23:45 에 19건 승인). 처음 둘은 환경변수로 걸었고, 세 번째에 **토요일 11:10 재발송 크론**으로 굳혔다. 이 환경변수는 이제 토요일까지도 못 기다릴 때만 쓴다.

```
TEXTBOOK_ONESHOT_AT=2026-08-23T11:05   # KST 벽시계, 분까지
```

- 지정 시각이 지나면 5분 크론이 잡아 `sendBatch()` 를 한 번 돌린다. **그날(KST) 안이면, 21시 전이면** 언제 서버가 떠도 첫 틱에 나간다. 🔴 원래는 "30분 안"이었는데 2026-09-14 에 원장이 `10:30` 으로 걸어 두고도 서버가 그 30분에 그 값을 들고 떠 있지 않아 안 나갔다(19가정 하루 밀림). 21시 컷은 밤에 입금 안내가 가는 걸 막는 것 — 21시 이후 시각은 걸 수도 없다(기동 로그 🔴)
- 🔴 **환경변수를 저장한 뒤 Manual Deploy 를 눌러야 한다.** 저장만으로는 돌고 있는 프로세스에 안 들어간다. 값은 `trim()` 하므로 끝에 공백·개행이 붙어도 된다
- 🔴 **`승인됨` 인 행만 나간다.** `승인대기` 는 아무리 걸어도 안 나간다 — 걸기 전에 승인 상태를 먼저 볼 것
- 실패하면 **자동 재시도하지 않는다.** 반쯤 나간 상태에서 다시 돌면 학부모가 두 번 받는다. 원장에게 카카오워크로 알린다
- 다 쓰면 환경변수를 지운다. 안 지워도 날이 지나면 다시 돌지 않는다
- 기동 로그로 상태를 확인한다: `⏰ …예약됨` / `⚠️ …이미 지났습니다` / `🔴 형식이 틀렸습니다` / `🔴 …21시 이후라 걸 수 없습니다`. **로그에 아무것도 없으면 재배포가 안 된 것이다**

## 주의

- 🔴 **크론은 서버 프로세스 안에 있다.** Render가 슬립하거나 재시작하면 그 시각 잡은 그냥 안 돈다. 재시도 큐가 없다.
- 🔴 **새 크론에 `{ timezone: 'Asia/Seoul' }`을 빠뜨리지 마라.** 지금 17개 전부 갖고 있다 (`test/textbook-fee.test.mjs` 가 교재비 것은 자동으로 검사한다).
- 크론 콜백 안의 맨 `new Date()`는 서버 시간이다 → [[kst-time]]
- 각 크론은 `try/catch`로 감싸져 있어 실패해도 다음 회차는 돈다. 다만 **실패가 콘솔에만 남는다** — 조용한 실패를 잡으려면 Render 로그를 봐야 한다.

관련: [[kst-time]] · [[textbook-fee]] · [[homework-automation]] · [[daily-report]] · [[absence-notice]] · [[counsel]] · [[arrival-alert]]
