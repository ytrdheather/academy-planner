...
🔴 **매월 1일 09:00 만 유일하게 월 단위다.** 나머지는 놓쳐도 하루~일주일 뒤에 다시 오지만 이건 한 달 뒤다. 배포와 겹치면 반드시 `POST /api/monthly-report/tick` 으로 복구한다 → [[monthly-report]]

교재비 담당쌤 알림은 **크론 두 개로 갈려 있다**(2026-09-04 원장 확정). 반려는 평일 14시에 빨리, 승인 결과는 금요일 학부모 발송 직후에 몰아서. 금요일 것은 크론을 새로 만들지 않고 21시 발송 콜백 끝에 이어 붙였다 — "발송 뒤"를 순서로 보장하려는 것이다. → [[textbook-fee]]
