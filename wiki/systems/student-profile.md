...
- 🔴 **연락처는 원장 계정 하나만 본다.** `loginId === 'manager'` (`api/studentProfileModule.js:210`). **`role === 'manager'` 를 쓰면 5명이 뚫린다** → [[role-manager-is-not-owner]]. 화면 가리기가 아니라 서버가 안 내려보낸다. 근거(2026-08-23 원장): 상담 DM은 사건마다 한 건씩 오지만 프로필은 **87명이 한번에 좍** 보인다 — 성격이 다르다. 담임은 그 건의 번호를 이미 DM으로 받는다 (`api/index.js:517`) → [[counsel]]
