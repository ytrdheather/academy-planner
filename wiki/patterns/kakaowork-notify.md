...
`sendKakaoWork(conversationId, text)` — `api/index.js:345`. `api.kakaowork.com/v1/messages.send`, `Bearer <APP_KEY>`.
...
- 모듈에는 채널이 이미 박힌 얇은 함수로 넘긴다 — `notifyChannel: (title, body) => sendKakaoWork(CONV, ...)` (`api/index.js:1167`). [[module-di]]
