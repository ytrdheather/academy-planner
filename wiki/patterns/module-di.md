...
type: pattern
status: verified
source: api/index.js:1078-1136, api/textbookFeeModule.js:41
updated: 2026-08-15
...
- **알림 함수는 얇게 감싸서 넘기는 게 관례다.** `confirmNotifyModule`은 `sendKakaoWork`를 직접 받지 않고 `notifyChannel(title, body)` 형태로 채널이 이미 박힌 함수를 받는다 (`api/index.js:1131`) — 모듈이 conversation ID를 몰라도 되게.
