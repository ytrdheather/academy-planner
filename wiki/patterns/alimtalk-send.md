...
`sendAlimtalk(to, templateId, variables)` — `api/index.js:370`
`sendSms(to, text, subject)` — `api/index.js:399`
...
1. 카카오에 템플릿 등록 → 심사 통과
2. 템플릿 ID를 **Render 환경변수**에 추가 (코드에 박지 마라). 없으면 발송만 건너뛰도록 폴백 `''`을 둔다 — `api/index.js:333` 참고
